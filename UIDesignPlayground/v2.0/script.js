...
class AudioVisualizer {
  constructor(container, config = {}) {
    ...
    // In AudioVisualizer constructor, after WebSocket setup:
    this.ws.onopen = () => {
    ...
    this.ws.onerror = (error) => {
      ...
      // Add state to status indicator
      const status = document.getElementById("status-indicator");
      if (status) {
        status.textContent = `Error: WebSocket issue`;
  ...
  async setMode(mode) {
    ...
    if (this.currentStream) {
      ...
      this.currentStream.getTracks().forEach((track) => {
        track.stop();
        console.log("🎤 Track stopped:", track.kind, track.enabled);
      });
    ...
    if (mode === "idle") {
      ...
      console.log(
        "🎤 Stream setup:",
        this.currentStream ? "success" : "failed"
      );
    }
  }
    ...
    if (transcriptArea) {
      ...
      if (this.interimTranscript) {
        ...
        interimSpan.style.color = getComputedStyle(document.documentElement)
          .getPropertyValue("--text-transcript-interim")
          .trim();
...
`;
  ...
  document
    .getElementById("send-transcript")
    .addEventListener("click", sendTranscript);
  document
    .getElementById("clear-transcript")
    .addEventListener("click", clearTranscript);
...
function toggleRecording() {
  ...
  if (isListening) {
    ...
    setButtonIcon(
      micButton,
      visualizer.isMuted ? "bi-mic-mute-fill" : "bi-mic-fill"
    );
