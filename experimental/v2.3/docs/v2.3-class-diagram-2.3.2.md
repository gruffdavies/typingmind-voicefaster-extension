...
classDiagram
    ...
    class QueueUIManager {
        ...
        +update(queue: TTSAudioQueue)
        -createQueueItem(stream: TTSQueueItem)
        -handleItemInteraction(item: HTMLElement)
    }

    class TTSAudioQueue {
        -streams: Array~TTSQueueItem~
        +addStream(stream: TTSQueueItem)
        +removeStream(id: string)
    }

    VoiceFasterController --> UIManager
    UIManager --> QueueUIManager
    QueueUIManager --> TTSAudioQueue : manages
```